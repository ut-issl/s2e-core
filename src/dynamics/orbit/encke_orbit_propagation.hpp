 ...
 * @brief Class to propagate spacecraft orbit with Encke's method
 */
class EnckeOrbitPropagation : public Orbit, public s2e::math::OrdinaryDifferentialEquation<6> {
 public:
  /**
  ...
  EnckeOrbitPropagation(const CelestialInformation* celestial_information, const double gravity_constant_m3_s2, const double propagation_step_s,
                        const double current_time_jd, const s2e::math::Vector<3> position_i_m, const s2e::math::Vector<3> velocity_i_m_s,
 ...
 private:
  ...
  s2e::math::Vector<3> difference_velocity_i_m_s_;  //!< Difference orbit velocity in the inertial frame [m/s]
   ...
   */
  void Initialize(const double current_time_jd, const s2e::math::Vector<3> reference_position_i_m, const s2e::math::Vector<3> reference_velocity_i_m_s);
  /**
   * @fn UpdateSatOrbit
