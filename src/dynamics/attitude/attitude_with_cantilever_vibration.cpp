...
AttitudeWithCantileverVibration::AttitudeWithCantileverVibration(
    ...
    const double intrinsic_angular_velocity_cantilever_rad_s, const s2e::math::Vector<3>& torque_b_Nm, const double propagation_step_s,
  ...
  s2e::math::Matrix<3, 3> inverse_equivalent_inertia_tensor_cantilever =
...
void AttitudeWithCantileverVibration::SetParameters(const MonteCarloSimulationExecutor& mc_simulator) {
  ...
  current_propagation_time_s_ = 0.0;
  angular_momentum_reaction_wheel_b_Nms_ = s2e::math::Vector<3>(0.0);  //!< TODO: Consider how to handle this variable
  CalcAngularMomentum();
}
...
void AttitudeWithCantileverVibration::Propagate(const double end_time_s) {
  ...
  s2e::math::Matrix<3, 3> previous_inertia_tensor_kgm2 = attitude_ode_.GetPreviousInertiaTensor_kgm2();
  assert(end_time_s - current_propagation_time_s_ > 1e-6);
  s2e::math::Matrix<3, 3> dot_inertia_tensor = (1.0 / (end_time_s - current_propagation_time_s_)) * (inertia_tensor_kgm2_ - previous_inertia_tensor_kgm2);
  s2e::math::Vector<3> torque_inertia_tensor_b_Nm = dot_inertia_tensor * angular_velocity_b_rad_s_;
