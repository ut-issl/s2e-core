...
Telescope::Telescope(ClockGenerator *clock_gen, libra::Quaternion &q_b2c,
    ...
    : ComponentBase(1, clock_gen), local_celes_info_(local_celes_info),
      attitude_(attitude), hipp_(hipp),
      sun_forbidden_angle_(sun_forbidden_angle),
      earth_forbidden_angle_(earth_forbidden_angle),
      moon_forbidden_angle_(moon_forbidden_angle), q_b2c_(q_b2c),
      x_num_of_pix_(x_num_of_pix), y_num_of_pix_(y_num_of_pix),
      x_fov_par_pix_(x_fov_par_pix), y_fov_par_pix_(y_fov_par_pix),
  ...
  assert(x_field_of_view_rad <
         M_PI / 2); //視野角90度以上だと計算が成り立たないので，その場合を弾く
...
void Telescope::MainRoutine(int count) {
  ...
  // = angle(sight_, sun_pos_c) * 180/M_PI; angle_earth = angle(sight_,
  // earth_pos_c) * 180 / M_PI; angle_moon = angle(sight_, moon_pos_c) * 180 /
  // M_PI;
  //******************************************************************************
}
