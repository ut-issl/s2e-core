...
PCU_InitialStudy::PCU_InitialStudy(ClockGenerator *clock_gen,
                                   const std::vector<SAP *> saps, BAT *bat)
    : ComponentBase(1000, clock_gen), saps_(saps), bat_(bat),
...
void PCU_InitialStudy::MainRoutine(int time_count) {
  power_consumption_ = CalcPowerConsumption(
      time_count * clock_gen_->IntervalMillisecond /
...
void PCU_InitialStudy::UpdateChargeCurrentAndBusVoltage() {
  ...
  double current_temp =
      (-bat_voltage + std::sqrt(bat_voltage * bat_voltage +
                                4.0 * bat_resistance *
                                    (power_generation - power_consumption_))) /
      (2.0 * bat_resistance);
  if (current_temp >= cc_charge_current_) {
    if (bat_voltage + cc_charge_current_ * bat_resistance <
        cv_charge_voltage_) {
      // CC Charge
      ...
      // CV Charge
      bat_->SetChargeCurrent((cv_charge_voltage_ - bat_voltage) /
                             bat_resistance);
      bus_voltage_ = bat_voltage + bat_resistance *
                                       (cv_charge_voltage_ - bat_voltage) /
                                       bat_resistance;
    }
  } else {
