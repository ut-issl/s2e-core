...
BAT::BAT(const BAT &obj)
  :ComponentBase(obj), number_of_series_(obj.number_of_series_), number_of_parallel_(obj.number_of_parallel_),
  cell_capacity_(obj.cell_capacity_), cell_discharge_curve_coeffs_(obj.cell_discharge_curve_coeffs_), dod_(obj.dod_),
  cc_charge_current_(obj.cc_charge_current_), cv_charge_voltage_(obj.cv_charge_voltage_), bat_resistance_(obj.bat_resistance_)
{
  charge_current_ = 0.0;
  UpdateBatVoltage();
}
...
void BAT::SetChargeCurrent(const double current)
{
  charge_current_ = current;
}

double BAT::GetBatVoltage() const
{
  return bat_voltage_;
}

double BAT::GetBatResistance() const
{
  return bat_resistance_;
}

double BAT::GetCCChargeCurrent() const
{
  return cc_charge_current_;
}

double BAT::GetCVChargeVoltage() const
{
  return cv_charge_voltage_;
}

string BAT::GetLogHeader() const
{
  string str_tmp = "";
