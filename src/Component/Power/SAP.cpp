...
SAP::SAP(ClockGenerator *clock_gen, int id, int number_of_series,
         int number_of_parallel, double cell_area,
         libra::Vector<3> normal_vector, double cell_efficiency,
         double transmission_efficiency, const SRPEnvironment *srp)
    : ComponentBase(1000, clock_gen), id_(id),
...
SAP::SAP(const SAP &obj)
    : ComponentBase(obj), id_(obj.id_),
      number_of_series_(obj.number_of_series_),
      number_of_parallel_(obj.number_of_parallel_), cell_area_(obj.cell_area_),
...
void SAP::MainRoutine(int time_count) {
  if (CsvScenarioInterface::IsCsvScenarioEnabled()) {
    ...
    libra::Vector<3> sun_direction_body =
        CsvScenarioInterface::GetSunDirectionBody(time_in_sec);
    libra::Vector<3> normalized_sun_direction_body =
        libra::normalize(sun_direction_body);
    power_generation_ =
        cell_efficiency_ * transmission_efficiency_ * solar_constant *
        (int)CsvScenarioInterface::GetSunFlag(time_in_sec) * cell_area_ *
        number_of_parallel_ * number_of_series_ *
    ...
    power_generation_ =
        cell_efficiency_ * transmission_efficiency_ * power_density *
        cell_area_ * number_of_parallel_ * number_of_series_ *
        inner_product(
            normal_vector_,
            normalized_sun_direction); //仮の実装．実際は太陽方向などからIVカーブを更新．動作電圧に応じた発電電力を求める
  }
  if (power_generation_ < 0)
    power_generation_ = 0.0;
}
