...
GNSSReceiver::GNSSReceiver(const int prescaler, ClockGenerator *clock_gen,
    ...
    : ComponentBase(prescaler, clock_gen), id_(id), gnss_id_(gnss_id),
      ch_max_(ch_max), antenna_model_(antenna_model),
      antenna_position_b_(ant_pos_b), q_b2c_(q_b2c), half_width_(half_width),
      nrs_eci_x_(0.0, noise_std[0], g_rand.MakeSeed()),
      nrs_eci_y_(0.0, noise_std[1], g_rand.MakeSeed()),
      nrs_eci_z_(0.0, noise_std[2], g_rand.MakeSeed()), dynamics_(dynamics),
      gnss_satellites_(gnss_satellites), simtime_(simtime) {}
GNSSReceiver::GNSSReceiver(const int prescaler, ClockGenerator *clock_gen,
    ...
    : ComponentBase(prescaler, clock_gen, power_port), id_(id),
      gnss_id_(gnss_id), ch_max_(ch_max), antenna_model_(antenna_model),
      antenna_position_b_(ant_pos_b), q_b2c_(q_b2c), half_width_(half_width),
      nrs_eci_x_(0.0, noise_std[0], g_rand.MakeSeed()),
      nrs_eci_y_(0.0, noise_std[1], g_rand.MakeSeed()),
      nrs_eci_z_(0.0, noise_std[2], g_rand.MakeSeed()), dynamics_(dynamics),
...
void GNSSReceiver::CheckAntennaCone(const Vector<3> pos_true_eci_,
  ...
  for (int i = 0; i < gnss_num; i++) {
    ...
    if (gnss_id_.find(id_tmp[0]) == std::string::npos)
      continue;
