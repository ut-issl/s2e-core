...
EMDS::EMDS(Vector<3> mm, Vector<3> displacement) : mm_(mm), dis_(displacement) {}
...
void EMDS::calc(Vector<3> d1_i, Vector<3> d2_i, Quaternion q1_ib, Quaternion q2_ib, Vector<3> r1_b, Vector<3> r2_b, double i1, double i2,
                Vector<3>* results) {
  if (i2 >= 0) {
  ...
  for (int i = 0; i < 4; i++) {
    if (norm(results[i]) >= clipThres) results[i] = clipThres * normalize(results[i]);
  }
}

void EMDS::calc_approx(Vector<3> d1_i, Vector<3> d2_i, Quaternion q1_ib, Quaternion q2_ib, Vector<3> r1_b, Vector<3> r2_b, double i1, double i2,
  ...
  Vector<3> mu1_i = dot(m1_b, n1) * n1 + cos(theta1) * (m1_b - dot(m1_b, n1) * n1) + sin(theta1) * cross(n1, m1_b);
  Vector<3> mu2_i = dot(m2_b, n2) * n2 + cos(theta2) * (m2_b - dot(m2_b, n2) * n2) + sin(theta2) * cross(n2, m2_b);

  // Coordinate transformation of position vector
  Vector<3> r1_i = dot(r1_b, n1) * n1 + cos(theta1) * (r1_b - dot(r1_b, n1) * n1) + sin(theta1) * cross(n1, r1_b);
  Vector<3> r2_i = dot(r2_b, n2) * n2 + cos(theta2) * (r2_b - dot(r2_b, n2) * n2) + sin(theta2) * cross(n2, r2_b);
  ...
  Vector<3> F1_i = 3.0 * mu_0 / (4.0 * pi * pow(norm(-dh), 4)) *
                   (dot(mu1_i, -d) * mu2_i + dot(mu2_i, -d) * mu1_i + dot(mu1_i, mu2_i) * (-d) - 5 * dot(mu1_i, -d) * dot(mu2_i, -d) * (-d));
  Vector<3> F2_i = 3.0 * mu_0 / (4.0 * pi * pow(norm(dh), 4)) *
                   (dot(mu2_i, d) * mu1_i + dot(mu1_i, d) * mu2_i + dot(mu2_i, mu1_i) * (d)-5 * dot(mu2_i, d) * dot(mu1_i, d) * (d));
