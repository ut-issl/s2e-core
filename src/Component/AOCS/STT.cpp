...
STT::STT(
  ...
  rot_(g_rand.MakeSeed()), n_ortho_(0.0, sigma_ortho, g_rand.MakeSeed()),n_sight_(0.0, sigma_sight, g_rand.MakeSeed()), 
  step_time_(step_time), output_delay_(output_delay),output_interval_(output_interval), pos_(0), count_(0), 
  sun_forbidden_angle_(sun_forbidden_angle), earth_forbidden_angle_(earth_forbidden_angle),
...
Quaternion STT::measure(const LocalCelestialInformation *local_celes_info, const Attitude *attinfo)
{
  ...
  if (++count_ == output_interval_){ count_ = 0; }
...
void STT::update(const LocalCelestialInformation *local_celes_info, const Attitude *attinfo)
{
  ...
  // Calc observed quaternion: Inertial frame → STT frame → Rotation around sight →Rotation around orthogonal direction
...
{
  Vector<3> vect1_normal(vect1); normalize(vect1_normal); //Normalize Vector1 
  Vector<3> vect2_normal(vect2); normalize(vect2_normal); //Normalize Vector2
