...
RWModel::RWModel(int prescaler, int fast_prescaler, ClockGenerator *clock_gen,
    ...
    : ComponentBase(prescaler, clock_gen, fast_prescaler),
      step_width_(step_width), dt_main_routine_(dt_main_routine),
      inertia_(inertia), max_torque_(max_torque),
      max_velocity_rpm_(max_velocity_rpm), q_b2c_(q_b2c), pos_b_(pos_b),
      dead_time_(dead_time), driving_lag_coef_(driving_lag_coef),
      coasting_lag_coef_(coasting_lag_coef),
      is_calculated_jitter_(is_calc_jitter_enabled),
      is_logged_jitter_(is_log_jitter_enabled), drive_flag_(drive_flag),
