  ...
  considers_structural_resonance_(considers_structural_resonance)
{
  ...
  if(considers_structural_resonance_){ CalcCoef(); }
}
...
void RWJitter::CalcJitter(double angular_velocity_rad)
{
  ...
  for (int i = 0; i < jitter_force_rot_phase_.size(); i++)
  {
    ...
    //Add jitter force
    unfiltered_jitter_force_n_c_[0] += radial_force_harmonics_coef_[i][1] * angular_velocity_rad * angular_velocity_rad * sin(jitter_force_rot_phase_[i]);
    unfiltered_jitter_force_n_c_[1] += radial_force_harmonics_coef_[i][1] * angular_velocity_rad * angular_velocity_rad * cos(jitter_force_rot_phase_[i]);
  ...
  for (int i = 0; i < jitter_torque_rot_phase_.size(); i++)
  {
    ...
    //Add jitter torque
    unfiltered_jitter_torque_n_c_[0] += radial_torque_harmonics_coef_[i][1] * angular_velocity_rad * angular_velocity_rad * sin(jitter_torque_rot_phase_[i]);
    unfiltered_jitter_torque_n_c_[1] += radial_torque_harmonics_coef_[i][1] * angular_velocity_rad * angular_velocity_rad * cos(jitter_torque_rot_phase_[i]);
  ...
  for (int i = 0; i < 3; i++)
  {
    filtered_jitter_force_n_c_[i] =
      (-coef_[1] * filtered_jitter_force_n_1_c_[i]
        ...
        + coef_[5] * unfiltered_jitter_force_n_2_c_[i]) / coef_[0];
    ...
    filtered_jitter_torque_n_c_[i] =
      (-coef_[1] * filtered_jitter_torque_n_1_c_[i]
        ...
        + coef_[5] * unfiltered_jitter_torque_n_2_c_[i]) / coef_[0];
  }
...
void RWJitter::CalcCoef()
{
  ...
  coef_[0] = 4.0 + 4.0*bandwidth_*damping_factor_*jitter_update_interval_*structural_resonance_angular_freq_ + pow(jitter_update_interval_, 2.0)*pow(structural_resonance_angular_freq_, 2.0);
  coef_[1] = -8.0 + 2.0*pow(jitter_update_interval_, 2.0)*pow(structural_resonance_angular_freq_, 2.0);
  coef_[2] = 4.0 - 4.0*bandwidth_*damping_factor_*jitter_update_interval_*structural_resonance_angular_freq_ + pow(jitter_update_interval_, 2.0)*pow(structural_resonance_angular_freq_, 2.0);
  coef_[3] = 4.0 + 4.0*damping_factor_*jitter_update_interval_*structural_resonance_angular_freq_ + pow(jitter_update_interval_, 2.0)*pow(structural_resonance_angular_freq_, 2.0);
  coef_[4] = -8.0 + 2.0*pow(jitter_update_interval_, 2.0)*pow(structural_resonance_angular_freq_, 2.0);
  coef_[5] = 4.0 - 4.0*damping_factor_*jitter_update_interval_*structural_resonance_angular_freq_ + pow(jitter_update_interval_, 2.0)*pow(structural_resonance_angular_freq_, 2.0);
}
