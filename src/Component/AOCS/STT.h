...
public:
  ...
  */
  STT(
    ...
    const double sigma_ortho,
    const double sigma_sight,
    const double step_time,
    const unsigned int output_delay,
    const unsigned int output_interval,
    const double sun_forbidden_angle,
    const double earth_forbidden_angle,
    const double moon_forbidden_angle,
...
protected:
  ...
  libra::Vector<3> ortho2_; // the secont orthogonal direction of sight at component frame
  ...
  unsigned int output_interval_; // output interval. unit: step_time,[0-MAX_DELAY]
  ...
  // Internal functions
  void update(const LocalCelestialInformation *local_celes_info, const Attitude *attinfo);  // update delay buffer
  libra::Quaternion measure(const LocalCelestialInformation *local_celes_info, const Attitude *attinfo); // Calc measured quaternion
  ...
  int EarthJudgement(const libra::Vector<3>& earth_b);  // Judge earth forbidden angle
  int MoonJudgement(const libra::Vector<3>& moon_b);    // Judge moon forbidden angle
  int CaptureRateJudgement(const libra::Vector<3>& omega_b);    // Judge limit angular velocity
