...
ExpHilsI2c::ExpHilsI2c(
  ...
  ObcI2cCommunicationBase(sils_port_id, i2c_address, obc, hils_port_id, hils_port_manager), mode_id_(mode_id),
...
void ExpHilsI2c::MainRoutine(int count)
{
  ...
  if (mode_id_ == 0) // master
  {
    ...
    std::cout << "master recieve: " << rx_data[0] << rx_data[1] << rx_data[2] << std::endl;
  ...
  else if (mode_id_ == 1) // slave
  {
    unsigned char rx_data[kCmdSize] = { 0 };
    ReadCommand(rx_data, kCmdSize);
    std::cout << "slave recieve: " << rx_data[0] << rx_data[1] << rx_data[2] << std::endl;
