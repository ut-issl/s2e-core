...
ObcI2cCommunicationBase::ObcI2cCommunicationBase(
    int port_id, const unsigned char i2c_address, OBC *obc)
...
void ObcI2cCommunicationBase::ReadRegister(const unsigned char reg_addr,
                                           unsigned char *data,
                                           const unsigned char len) {
  obc_->I2cComponentReadRegister(port_id_, i2c_address_, reg_addr, data, len);
}

void ObcI2cCommunicationBase::WriteRegister(const unsigned char reg_addr,
                                            const unsigned char *data,
                                            const unsigned char len) {
  obc_->I2cComponentWriteRegister(port_id_, i2c_address_, reg_addr, data, len);
}

void ObcI2cCommunicationBase::ReadCommand(unsigned char *data,
                                          const unsigned char len) {
