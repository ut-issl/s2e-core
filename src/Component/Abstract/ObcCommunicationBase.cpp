...
void ObcCommunicationBase::InitializeObcComBase() {
  ...
  switch (sim_mode_) {
    ...
    case OBC_COM_UART_MODE::SILS:
      ...
      break;
    case OBC_COM_UART_MODE::HILS:
      ret = hils_port_manager_->UartConnectComPort(hils_port_id_, baud_rate_,
                                                   tx_buf_size_, rx_buf_size_);
      if (ret != 0) {
        std::cout << "Error: ObcCommunication ConnectComPort ID:"
                  << hils_port_id_ << "\n";
        is_connected_ = false;
      } else {
        is_connected_ = true;
...
int ObcCommunicationBase::ReceiveCommand(const int offset, const int rec_size) {
  ...
  switch (sim_mode_) {
    case OBC_COM_UART_MODE::SILS:
      ret = obc_->ReceivedByCompo(sils_port_id_, &rx_buffer_.front(), offset,
                                  rec_size);
      if (ret == 0) return 0;  // No read data
      return ParseCommand(ret);
    case OBC_COM_UART_MODE::HILS:
      ret = hils_port_manager_->UartReceive(hils_port_id_, &rx_buffer_.front(),
                                            offset, rec_size);
      if (ret == 0) return 0;  // No read data
      return ParseCommand(ret);
    default:
