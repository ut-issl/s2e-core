...
bool ObcGpioBase::Read(const int idx)
{
  return obc_->GpioComponentRead(port_id_[idx]);
}

void ObcGpioBase::Write(const int idx, const bool is_high)
{
  obc_->GpioComponentWrite(port_id_[idx], is_high);
}
