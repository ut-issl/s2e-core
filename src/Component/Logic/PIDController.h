...
template <size_t N> class PIDController {
public:
...
template <size_t N> PIDController<N>::PIDController(double dt_) {
  dt = dt_;
  last_p = Vector<N>(0);
  term_i = Vector<N>(0);
}

template <size_t N> PIDController<N>::~PIDController() {}

template <size_t N> Vector<N> PIDController<N>::CalcOutput(Vector<N> p_error) {
  if (isFirst) {
    isFirst = false;
    last_p = p_error;
  }
