...
GScalculator::GScalculator(double loss_polarization,
...
bool GScalculator::IsVisible(const Dynamics& dynamics, const GroundStation& groundstation)
{
  ...
  //std::cout << std::asin(dot(sc_pos_ltc, dir_GS_to_zenith) / norm(sc_pos_ltc)) / DEG2RAD << std::endl;
  return true;
  } else{
    return false;
  }
...
double GScalculator::CalcMaxBitrate(const Dynamics& dynamics, const ANT& sc_ant, const GroundStation& groundstation, const ANT& gs_ant)
{
  ...
  double CN0 = sc_ant.GetTxEIRP(sc_boresight_angle) + loss_space + loss_polarization_ + loss_atmosphere_ + loss_rainfall_ + loss_others_ + gs_ant.GetRxGT(gs_boresight_angle) - 10 * log10(Kb);  //[dBHz]
