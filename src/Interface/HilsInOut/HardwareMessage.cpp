...
int HardwareMessage::Initialize()
{
  ...
  {
    // ポート初期設定失敗
    //printf((hw_msg->hils_com_port->kPortName + "の初期設定に失敗しました。").c_str());
    printf("COMポートの初期設定に失敗しました。");
  }
  else if (ret < -1)
  {
    // ポートOPEN失敗
    //printf((hw_msg->hils_com_port->kPortName + "のOPENに失敗しました。").c_str());
    printf("COMポートのOPENに失敗しました。");
  }
  //printf((hw_msg->hils_com_port->kPortName + "のOPENに成功しました。\n").c_str());
...
int HardwareMessage::ReceiveAndInterpretMsg()
{
  ...
  if (offset != 0) hils_com_port_->Receive(rxbuff, kRxMessageSize, offset);
  ...
  if (memcmp(footer_ptr, kFooter, kFooterSize)) return -1;
  ...
  memcpy(&(hw_msg_.header), ptr, size); ptr += size;

  // OBC TI
  size = sizeof(hw_msg_.obc_ti);
  endian_memcpy(&(hw_msg_.obc_ti), ptr, size); ptr += size;
  printf("\rOBC TI: %d\n", hw_msg_.obc_ti);  // for debug

  // SADA
  size = sizeof(hw_msg_.sada.limit_switch_status);
  endian_memcpy(&(hw_msg_.sada.limit_switch_status), ptr, size); ptr += size;

  size = sizeof(hw_msg_.sada.position_steps);
  endian_memcpy(&(hw_msg_.sada.position_steps), ptr, size); ptr += size;

  size = sizeof(hw_msg_.sada.position_degrees);
  endian_memcpy(&(hw_msg_.sada.position_degrees), ptr, size); ptr += size;

  size = sizeof(hw_msg_.sada.step_period);
  endian_memcpy(&(hw_msg_.sada.step_period), ptr, size); ptr += size;

  size = sizeof(hw_msg_.sada.steps_remaining);
  endian_memcpy(&(hw_msg_.sada.steps_remaining), ptr, size); ptr += size;

  size = sizeof(hw_msg_.sada.steps_performed);
  endian_memcpy(&(hw_msg_.sada.steps_performed), ptr, size); ptr += size;

  size = sizeof(hw_msg_.sada.steps_since_home);
  endian_memcpy(&(hw_msg_.sada.steps_since_home), ptr, size); ptr += size;

  size = sizeof(hw_msg_.sada.spare);
  memcpy(&(hw_msg_.sada.spare), ptr, size); ptr += size;

  // Valve control
  size = sizeof(hw_msg_.valve_ctrl.TV_idx);
  endian_memcpy(&(hw_msg_.valve_ctrl.TV_idx), ptr, size); ptr += size;
  printf("\rTV_idx: %d\n", hw_msg_.valve_ctrl.TV_idx);

  size = sizeof(hw_msg_.valve_ctrl.cumulative_rv_open_counter);
  endian_memcpy(&(hw_msg_.valve_ctrl.cumulative_rv_open_counter), ptr, size); ptr += size;
  ...
  for (int i = 0; i < VALVE_THRUSTER_MAX; i++)
  {
    ...
    endian_memcpy(&(hw_msg_.valve_ctrl.thruster_thrust[i]), ptr, size); ptr += size;
	printf("%f\t", hw_msg_.valve_ctrl.thruster_thrust[i]);
  }
  printf("\n");
  
  size = sizeof(hw_msg_.valve_ctrl.spare);
  memcpy(&(hw_msg_.valve_ctrl.spare), ptr, size); ptr += size;

  // Thermal control
  size = sizeof(hw_msg_.thermal_ctrl.spare);
  memcpy(&(hw_msg_.thermal_ctrl.spare), ptr, size); ptr += size;

  // Spare
  size = sizeof(hw_msg_.spare);
  memcpy(&(hw_msg_.spare), ptr, size); ptr += size;

  // CRC
  size = sizeof(hw_msg_.crc);
  endian_memcpy(&(hw_msg_.crc), ptr, size); ptr += size; // memcpyではない？

  // Footer
  size = sizeof(hw_msg_.footer);
  memcpy(&(hw_msg_.footer), ptr, size); ptr += size;

  return 0;
}


bool HardwareMessage::Enable() const
{
  return enable_;
}
