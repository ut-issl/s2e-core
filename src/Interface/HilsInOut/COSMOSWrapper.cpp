...
COSMOSWrapper::COSMOSWrapper(bool enable)
  :enable_(enable)
{
...
COSMOSWrapper::COSMOSWrapper(unsigned short port_num, bool enable, char* server_ip)
...
void COSMOSWrapper::Finalize()
{
  COSMOS_TCP_IF::Finalize();
}

bool COSMOSWrapper::Enable() const
{
  return enable_;
}
...
void COSMOSWrapper::Cmd_RDP_SAT_CMD_EXTERNAL_TORQUE(uint8_t torque_frame, uint32_t duration_ms, float ext_torque_x, float ext_torque_y, float ext_torque_z)
{
  if (!enable_) return;


  // cmd("SAT SAT_CMD_EXTERNAL_TORQUE with SYNC_HI 3162369199, SYNC_LO 3737844679, LEN 21, AP_ID 2, OP_CODE 1, TORQUE_FRAME 'BODY', SPARE 0, DURATION_MS 1000, EXT_TORQUE_X 0.1, EXT_TORQUE_Y 0, EXT_TORQUE_Z 0")
  //std::string cmd_str = "SAT SAT_CMD_EXTERNAL_TORQUE with TORQUE_FRAME ";
  std::string cmd_str = "SAT SAT_CMD_EXTERNAL_TORQUE with SYNC_HI 3162369199, SYNC_LO 3737844679, LEN 21, AP_ID 2, OP_CODE 1, TORQUE_FRAME 'BODY', SPARE 0";
