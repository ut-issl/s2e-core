...
Attitude *InitAttitude(std::string file_name, const Orbit *orbit,
  ...
  if (propagate_mode == 0) {
    ...
    std::string name =
        section_ + std::to_string(sat_id); // "Attitude" + to_string(id);
    attitude = new AttitudeRK4(omega_b, quaternion_i2b, inertia_tensor,
                               torque_b, step_sec, name);
  }
  // Controlled attitude
  else {
    // new file open
    ...
    AttCtrlMode main_mode =
        static_cast<AttCtrlMode>(ini_file_ca.ReadInt(section_ca_, "main_mode"));
    AttCtrlMode sub_mode =
        static_cast<AttCtrlMode>(ini_file_ca.ReadInt(section_ca_, "sub_mode"));
    ...
    attitude = new ControlledAttitude(main_mode, sub_mode, quaternion_i2t,
                                      pointing_t_b, pointing_sub_t_b,
                                      celes_info, orbit);
  }
