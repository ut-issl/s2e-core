...
void InitParams(int actuator_id, std::string file_name, double prop_step,
                double compo_update_step) {
  // Access Parameters
  IniAccess rwmodel_conf(file_name);
  const std::string st_actuator_num =
      std::to_string(static_cast<long long>(actuator_id));
  ...
  direction_determination_mode =
      rwmodel_conf.ReadString(RWsection, "direction_determination_mode");
  ...
  is_calc_jitter_enabled =
      rwmodel_conf.ReadEnable(RWsection, "jitter_calculation");
  is_log_jitter_enabled = rwmodel_conf.ReadEnable(RWsection, "jitter_logging");

  std::string radial_force_harmonics_coef_path =
      rwmodel_conf.ReadString(RWsection, "radial_force_harmonics_coef_path");
  std::string radial_torque_harmonics_coef_path =
      rwmodel_conf.ReadString(RWsection, "radial_torque_harmonics_coef_path");
  ...
  conf_radial_force_harmonics.ReadCsvDouble(radial_force_harmonics_coef,
                                            harmonics_degree);
  conf_radial_torque_harmonics.ReadCsvDouble(radial_torque_harmonics_coef,
                                             harmonics_degree);

  structural_resonance_freq =
      rwmodel_conf.ReadDouble(RWsection, "structural_resonance_freq");
  ...
  considers_structural_resonance =
      rwmodel_conf.ReadEnable(RWsection, "considers_structural_resonance");
...
RWModel InitRWModel(ClockGenerator *clock_gen, int actuator_id,
  ...
  RWModel rwmodel(
      prescaler, fast_prescaler, clock_gen, step_width, dt_main_routine,
      jitter_update_interval, inertia, max_torque, max_velocity, q_b2c, pos_b,
      dead_time, ordinary_lag_coef, coasting_lag_coef, is_calc_jitter_enabled,
      is_log_jitter_enabled, radial_force_harmonics_coef,
      radial_torque_harmonics_coef, structural_resonance_freq, damping_factor,
...
RWModel InitRWModel(ClockGenerator *clock_gen, PowerPort *power_port,
  ...
  RWModel rwmodel(prescaler, fast_prescaler, clock_gen, power_port, step_width,
                  dt_main_routine, jitter_update_interval, inertia, max_torque,
                  max_velocity, q_b2c, pos_b, dead_time, ordinary_lag_coef,
                  coasting_lag_coef, is_calc_jitter_enabled,
                  is_log_jitter_enabled, radial_force_harmonics_coef,
                  radial_torque_harmonics_coef, structural_resonance_freq,
