...
MagTorquer InitMagTorquer(ClockGenerator* clock_gen, int actuator_id, const string fname, double compo_step_time, const MagEnvironment* mag_env)
{
  ...
  MagTorquer magtorquer(prescaler, clock_gen, actuator_id, 
          q_b2c, scale_factor, max_c, min_c, bias_c, 
          rw_stepwidth, rw_stddev_c, rw_limit_c, nr_stddev_c, mag_env);
  return magtorquer;	
}
...
MagTorquer InitMagTorquer(ClockGenerator* clock_gen, PowerPort* power_port, int actuator_id, const string fname, double compo_step_time, const MagEnvironment* mag_env)
{
  ...
  // PowerPort
  double minimum_voltage = magtorquer_conf.ReadDouble(MTSection, "minimum_voltage");
  power_port->SetMinimumVoltage(minimum_voltage);
  double assumed_power_consumption = magtorquer_conf.ReadDouble(MTSection, "assumed_power_consumption");
  power_port->SetAssumedPowerConsumption(assumed_power_consumption);

  MagTorquer magtorquer(prescaler, clock_gen, power_port, actuator_id, 
          q_b2c, scale_factor, max_c, min_c, bias_c, 
          rw_stepwidth, rw_stddev_c, rw_limit_c, nr_stddev_c, mag_env);
  return magtorquer;	
}