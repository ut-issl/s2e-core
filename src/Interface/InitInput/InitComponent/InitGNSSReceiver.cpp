...
GNSSReceiverParam ReadGNSSReceiverIni(const std::string fname,
  ...
  gnssreceiver_param.antenna_model =
      static_cast<AntennaModel>(gnssr_conf.ReadInt(GSection, "antenna_model"));
  if (!gnss_satellites->IsCalcEnabled() &&
      gnssreceiver_param.antenna_model == CONE) {
  ...
  gnssr_conf.ReadVector(GSection, "antenna_pos_b",
                        gnssreceiver_param.antenna_pos_b);
  ...
  gnssr_conf.ReadVector(GSection, "nr_stddev_eci",
                        gnssreceiver_param.noise_std);
...
GNSSReceiver InitGNSSReceiver(ClockGenerator* clock_gen, int id,
  ...
  GNSSReceiver gnss_r(gr_param.prescaler, clock_gen, id, gr_param.gnss_id,
                      gr_param.ch_max, gr_param.antenna_model,
                      gr_param.antenna_pos_b, gr_param.q_b2c,
                      gr_param.half_width, gr_param.noise_std, dynamics,
                      gnss_satellites, simtime);
  return gnss_r;
};
...
GNSSReceiver InitGNSSReceiver(ClockGenerator* clock_gen, PowerPort* power_port,
  ...
  GNSSReceiver gnss_r(gr_param.prescaler, clock_gen, power_port, id,
                      gr_param.gnss_id, gr_param.ch_max, gr_param.antenna_model,
                      gr_param.antenna_pos_b, gr_param.q_b2c,
                      gr_param.half_width, gr_param.noise_std, dynamics,
                      gnss_satellites, simtime);
  return gnss_r;
