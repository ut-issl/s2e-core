...
Orbit* InitOrbit(const CelestialInformation* celes_info, std::string ini_path, double stepSec, double current_jd, double gravity_constant, std::string section, RelativeInformation* rel_info)
{
  ...
  {
    int wgs = conf.ReadInt(section_, "wgs");
    RelativeOrbit::RelativeOrbitUpdateMethod update_method = (RelativeOrbit::RelativeOrbitUpdateMethod)(conf.ReadInt(section_, "relative_orbit_update_method"));
    ...
    // There is a possibility that the orbit of the reference sat is not initialized when RelativeOrbit initialization is called
    // To ensure that the orbit of the reference sat is initialized, create temporary initial orbit of the reference sat 
