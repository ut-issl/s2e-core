...
PowerPort::PowerPort()
    : kPortId(-1), current_limit_(10.0), minimum_voltage_(3.3),
...
PowerPort::PowerPort(int port_id, double current_Limit)
    : kPortId(port_id), current_limit_(current_Limit), minimum_voltage_(3.3),
      assumed_power_consumption_(0.0) {
  Initialize();
}
...
PowerPort::PowerPort(int port_id, double current_Limit, double minimum_voltage,
    ...
    : kPortId(port_id), current_limit_(current_Limit),
...
void PowerPort::SubtractAssumedPowerConsumption(const double power) {
  ...
  if (assumed_power_consumption_ < 0.0)
    assumed_power_consumption_ = 0.0;
  return;
}