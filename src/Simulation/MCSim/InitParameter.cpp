...
void InitParameter::gen_NoRandomization()
{
  val_.clear();
}
...
void InitParameter::get_SphericalNormalUniformUniform(Vector<3>& dst, double r_mean, double r_sigma, double theta_min, double theta_max, double phi_min, double phi_max)
{
  double r = InitParameter::Normal_1d(r_mean, r_sigma);
...
void InitParameter::gen_SphericalNormalUniformUniform()
{
  ...
  Vector<dim> temp_vec;
  get_SphericalNormalUniformUniform(temp_vec, mean_or_min_[0], sigma_or_max_[0], mean_or_min_[1], sigma_or_max_[1], mean_or_min_[2], sigma_or_max_[2]);
...
void InitParameter::get_SphericalNormalNormal(Vector<3>& dst, const Vector<3>& mean_vec, double r_sigma, double theta_sigma)
{
  ...
  Quaternion rotation_of_mean_vec(rotation_axis, -rotation_angle_of_mean_vec); //座標が回転するのではなくベクトルが回転するので角度の向きを逆にする
