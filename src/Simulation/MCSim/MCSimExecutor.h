...
public:
  ...
  // InitParameterの追加
  void AddInitParameter(std::string so_name, std::string ip_name,
                        const Vector<NumElement1> &mean_or_min,
  ...
  void GetInitParameterVec(std::string so_name, std::string ip_name,
                           Vector<NumElement> &dst_vec) const;

  // Randomizeされた後の値を取得しdstに格納
  void GetInitParameterDouble(std::string so_name, std::string ip_name,
                              double &dst) const;

  // Randomizeされた後の値を取得しdst_quatに格納
  void GetInitParameterQuaternion(std::string so_name, std::string ip_name,
...
void MCSimExecutor::SetTotalNumOfExecutions(
    unsigned long long num_of_executions) {
...
template <size_t NumElement>
void MCSimExecutor::GetInitParameterVec(std::string so_name,
                                        std::string ip_name,
                                        Vector<NumElement> &dst_vec) const {
  if (!enabled_)
    return;
...
template <size_t NumElement1, size_t NumElement2>
void MCSimExecutor::AddInitParameter(
    std::string so_name, std::string ip_name,
    const Vector<NumElement1> &mean_or_min,
    const Vector<NumElement2> &sigma_or_max,
    InitParameter::RandomizationType rnd_type) {
  std::string name = so_name + MCSimExecutor::separator_ + ip_name;
