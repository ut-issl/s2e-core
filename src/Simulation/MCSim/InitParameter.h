...
public:
  ...
  enum RandomizationType {
    ...
    SphericalNormalNormal,         // ベクトルのノルム(r)とデフォルトベクトルとの角度(θ)が正規分布に従う．mean vector周りの角度(φ)は[0,2*pi]の一様分布．
...
private:
  ...
  void get_SphericalNormalUniformUniform(Vector<3>& dst, double r_mean, double r_sigma, double theta_min, double theta_max, double phi_min, double phi_max);
...
template<size_t NumElement1, size_t NumElement2>
void InitParameter::SetRandomConfig(const Vector<NumElement1>& mean_or_min, const Vector<NumElement2>& sigma_or_max, InitParameter::RandomizationType rnd_type)
{
  rnd_type_ = rnd_type;
  mean_or_min_.clear();
