...
SampleGSComponents::SampleGSComponents(const SimulationConfig* config) : config_(config) {
  ...
  std::string ant_ini_path = iniAccess.ReadString("COMPONENTS_FILE", "ground_station_antenna_file");
  config_->main_logger_->CopyFileToLogDir(ant_ini_path);
  antenna_ = new Antenna(InitAntenna(1, ant_ini_path));
  std::string gscalculator_ini_path = iniAccess.ReadString("COMPONENTS_FILE", "ground_station_calculator_file");
