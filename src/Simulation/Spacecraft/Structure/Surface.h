...
class Surface {
 public:
  Surface(Vector<3> position, Vector<3> normal, double area,
          double reflectivity, double specularity, double air_specularity);
  ~Surface(){};
  ...
  inline const double& GetAirSpecularity(void) const {
    return air_specularity_;
  }
 ...
 private:
  ...
  double specularity_;   // Ratio of specular reflection in the total reflected
                         // light
