...
void gigrf(int gen, double year)
{
  ...
  if ((pstr = strstr(file, "13")) == NULL)
	...
	//sprintf(char *str, const char *format,  ．．． )=printf関数と同様の変換を行った出力を、文字列strに格納
	//char *strncpy(char *s1, const char *s2, size_t n);= *s1 に文字列 *s2 を先頭から n 文字コピー
	...
	//int sscanf(const char *str, const char *format,  ．．． );=strから書式formatにしたがって、scanf関数と同様の変換を行った入力を、指定されたアドレスに格納
	...
	maxod = k;
	//ジオイドに関わる定数?? field(double are, double aflat, double ara, int maxoda)
...
void IgrfCalc(double decyear, double latrad, double lonrad, double alt, double side, double* mag)
{
	...
	igrfc(latrad*RAD2DEG, lonrad*RAD2DEG, alt, &f);	//実行位置の設定&Executeはこの中に含む
