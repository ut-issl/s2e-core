...
double angle(double vec1[3], double vec2[3]) {
  ...
  if (magv1 * magv2 > small * small) {
    ...
    if (fabs(temp) > 1.0) temp = sgn(temp) * 1.0;
    return acos(temp);
  } else
    return undefined;
}  // end angle
 ...
 *	真近点離角がわかっているときにケプラー方程式を解く。
 *	平均近点角、離心率、双曲線、楕円アノマリーもわかる。
 *
 ...
 *  この関数では地心の位置と速度ベクトルから古典的な軌道要素を求める。
 *
 ...
 *    arglat      - argument of latitude      (ci) 0.0  to 2pi rad	//緯度
 *    truelon     - true longitude            (ce) 0.0  to 2pi rad
 * //真の経度 lonper      - longitude of periapsis    (ee) 0.0  to 2pi rad
 * //近地点での経度？
 *
 *  locals        :
 *    hbar        - angular momentum h vector      km2 / s
 *    ebar        - eccentricity     e vector
 * //離心率のベクトル？ nbar        - line of nodes    n vector c1          -
 * v**2 - u/r rdotv       - r dot v hk          - hk unit vector sme         -
 * specfic mechanical energy      km2 / s2 i           - index e           -
 * eccentric, parabolic, hyperbolic anomaly             rad temp        -
 * temporary variable typeorbit   - type of orbit                  ee, ei, ce,
 * ci
 *
 *  coupling      :
 *    mag         - magnitude of a vector
...
void rv2coe(double r[3], double v[3], double mu, double& p, double& a,
  ...
  if (magh > small) {
    ...
    rdotv = dot(r, v);
    for (i = 0; i <= 2; i++) ebar[i] = (c1 * r[i] - rdotv * v[i]) / mu;
    ecc = mag(ebar);
    ...
    if (ecc < small) {
      ...
      if ((incl < small) | (fabs(incl - pi) < small)) strcpy(typeorbit, "ee");
    }
    ...
    if (magn > small) {
      ...
      omega = acos(temp);
      if (nbar[1] < 0.0) omega = twopi - omega;
    } else
      omega = undefined;
    ...
    if ((ecc > small) && (strcmp(typeorbit, "ee") == 0)) {
      ...
      if (fabs(temp) > 1.0) temp = sgn(temp);
      lonper = acos(temp);
      if (ebar[1] < 0.0) lonper = twopi - lonper;
      if (incl > halfpi) lonper = twopi - lonper;
    } else
      lonper = undefined;
    ...
    if ((magr > small) && (strcmp(typeorbit, "ce") == 0)) {
      ...
      truelon = acos(temp);
      if (r[1] < 0.0) truelon = twopi - truelon;
      if (incl > halfpi) truelon = twopi - truelon;
      m = truelon;
    } else
      truelon = undefined;

    // ------------ find mean anomaly for all orbits -----------
    if (typeorbit[0] == 'e') newtonnu(ecc, nu, e, m);
  } else {
    p = undefined;
    a = undefined;
    ecc = undefined;
 ...
 *
 *  coupling      :
 *    days2mdhms  - finds month, day, hour, minute and second given days and
 *year
 *
 *  references    :
