...
GaussianBeamBase::GaussianBeamBase(double wavelength_m, double r_beam_waist_m, double total_power_watt)
    : wavelength_m_(wavelength_m), r_beam_waist_m_(r_beam_waist_m), total_power_watt_(total_power_watt) {}
...
void GaussianBeamBase::SetBeamWaistPos_i(const libra::Vector<3> pos_beamwaist_i) { pos_beamwaist_i_ = pos_beamwaist_i; }

double GaussianBeamBase::CalcBeamWidthRadius(double distance_from_beamwaist_m) {
  double rayleigh_length_m = libra::pi * r_beam_waist_m_ * r_beam_waist_m_ / wavelength_m_;
...
double GaussianBeamBase::CalcIntensity(double distance_from_beamwaist_m, double deviation_from_optical_axis_m) {
  ...
  double peak_intensity_watt_m2 = (2.0 * total_power_watt_) / (libra::pi * beam_width_radius_m * beam_width_radius_m);
