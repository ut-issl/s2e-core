...
namespace libra
{
  ...
  Vector<R, TC> operator*(const Matrix<R, C, TM>& m,
			  const Vector<C, TC>& v)
  {
  ...
  template<std::size_t N>
	Matrix<N, N> invert(const Matrix<N, N>& a)
	{
		...
		{
			fill_up(v, 0.0); v[i] = 1.0;
			lubksb(temp, index, v);
			for(unsigned int j=0; j<N; ++j){ inv[j][i] = v[j]; }
		}
		return inv;
	}

  template<std::size_t N>
	Matrix<N, N>& ludcmp(Matrix<N, N>& a,
											 unsigned int index[])
	{
		double coef[N];
		for(unsigned int i=0; i<N; ++i)
		...
		for(unsigned int j=0; j<N; ++j)
		{
			...
			for(unsigned int i=j; i<N; ++i)
			{
				...
				double temp;
				if((temp = coef[i]*fabs(sum)) >= biggest){ biggest = temp; imax = i; }
			}
			...
			if(j != N)
			{
				...
				for(unsigned int i=j+1; i<N; ++i){ a[i][j] *= temp; }
			}
		}
		return a;
	}
  ...
  template<std::size_t N>
	Vector<N>& lubksb(const Matrix<N, N>& a,
										const unsigned int index[],
										Vector<N>& b)
	{
		double sum;
		bool non_zero = false; unsigned int mark;
		for(unsigned int i=0; i<N; ++i)
		{
			...
			if(non_zero)
			{
				for(unsigned int j=mark; j<i; ++j){ sum -= a[i][j]*b[j]; }
			}
			else if(sum != 0.0)
			{
				non_zero = true; mark = i;
			}
			b[i] = sum;
		}
