...
template<size_t N>
RandomWalk<N>::RandomWalk(double step_width,
...
template<size_t N>
void RandomWalk<N>::RHS(double x,
            const libra::Vector<N>& state,
            libra::Vector<N>& rhs)
{
  for(size_t i=0; i<N; ++i)
  {
    if(state[i] > limit_[i])        rhs[i] = -fabs(nrs_[i]); 
    else if(state[i] < -limit_[i])  rhs[i] = fabs(nrs_[i]);
    else                            rhs[i] = nrs_[i];
  }
}