...
{
  stream << v[0];
  for(int i=1; i<N; ++i){ stream << delimiter << v[i]; }
}
...
{
  Vector<N, T> temp;
  for(size_t i=0; i<N; ++i){ temp[i] = lhs[i]+rhs[i]; }
  return temp;
}
...
{
  Vector<N, T> temp;
  for(int i=0; i<N; ++i){ temp[i] = lhs[i]-rhs[i]; }
  return temp;
}
...
{
  Vector<N, T> temp;
  for(size_t i=0; i<N; ++i){ temp[i] = lhs*rhs[i]; }
  return temp;
}
...
{
  T temp = 0;
  for(int i=0; i<N; ++i){ temp += lhs[i]*rhs[i]; }
  return temp;
}
...
{
  double temp = 0.0;
  for(size_t i=0; i<N; ++i){ temp += pow(v[i], 2.0); }
  return sqrt(temp);
}
...
{
  double n = norm(v);
  if(n == 0.0){ return v; } //零ベクトル

  // 正規化
  n = 1.0/n;
  for(size_t i=0; i<N; ++i){ v[i]*=n; }
