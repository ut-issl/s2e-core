...
void ODE<N>::Update()
{
  RHS(x_, state_, rhs_); //Current derivative calculation
                         ...
                         // 4次のRunge-Kutta係数計算
  Vector<N> k1(rhs_); k1 *= step_width_;
  Vector<N> k2(state_.dim());
  RHS(x_ + 0.5*step_width_, state_ + 0.5*k1, k2); k2 *= step_width_;
  Vector<N> k3(state_.dim());
  RHS(x_ + 0.5*step_width_, state_ + 0.5*k2, k3); k3 *= step_width_;
  Vector<N> k4(state_.dim());
  RHS(x_ + step_width_, state_ + k3, k4); k4 *= step_width_;
