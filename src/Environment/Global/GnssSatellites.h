...
/**
 * @enum UR_KINDS
 * @enum When Using Ultra Rapid Calendar, decide to use which 6 hours in each observe and predict 24 hours calender
 */
typedef enum
{
...
class GnssSat_coordinate
{
  ...
  protected:
    ...
    */
    template<size_t N> libra::Vector<N> TrigonometricInterpolation(const std::vector<double>& time_vector, const std::vector<libra::Vector<N>>& values, double time) const;
    double TrigonometricInterpolation(const std::vector<double>& time_vector, const std::vector<double>& values, double time) const;
    template<size_t N> libra::Vector<N> LagrangeInterpolation(const std::vector<double>& time_vector, const std::vector<libra::Vector<N>>& values, double time) const;
...
class GnssSat_position: public GnssSat_coordinate
{
  public:
    GnssSat_position() {}
    std::pair<double, double> Init(std::vector<std::vector<std::string>>& file, int interpolation_method,
...
class GnssSat_clock: public GnssSat_coordinate
{
  public:
    ...
    void Init(std::vector<std::vector<std::string>>& file, std::string file_extension,
              int interpolation_number, UR_KINDS ur_flag, std::pair<double, double> unix_time_period);
    void SetUp(const double start_unix_time, const double step_sec);
    void Update(const double now_unix_time);
...
public:
  ...
  //Get observation Range:[m] CarrierPhase:[no unit], frequency is thought to be expressed in MHz
