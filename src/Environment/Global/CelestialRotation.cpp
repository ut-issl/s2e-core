...
void CelestialRotation::Init_CelestialRotation_As_Earth(const RotationMode rotation_mode, const string center_obj)
{
  ...
  if (center_obj == planet_name_)
  {
    ...
    else if (rotation_mode == Full)
    {
      ...
      // the actual unit of c_epsi_rad_ is [rad/century^i], i is the index of the array
      ...
      // the actual unit of c_lm_rad_ is [rad/century^i], i is the index of the array
      ...
      // the actual unit of c_ls_rad_ is [rad/century^i], i is the index of the array
      ...
      // F means mean longitude of the moon - mean longitude of ascending node of the moon
      // the actual unit of c_F_rad_ is [rad/century^i], i is the index of the array
      ...
      // the actual unit of c_D_rad_ is [rad/century^i], i is the index of the array
      ...
      // the actual unit of c_O_rad_ is [rad/century^i], i is the index of the array
      ...
      // the actual unit of c_zeta_rad_ is [rad/century^(i+1)], i is the index of the array
      ...
      // the actual unit of c_theta_rad_ is [rad/century^(i+1)], i is the index of the array
...
void CelestialRotation::Update(const double JulianDate)
{
  ...
  if (rotation_mode_ == Full)
  {
    ...
    for (int i = 0; i < 3; i++) { tTT_century[i+1] = tTT_century[i] * tTT_century[0]; }

    Matrix<3, 3> P; Matrix<3, 3> N; Matrix<3, 3> R; Matrix<3, 3> W;
    // Nutation + Precession
    P = Precession(tTT_century);
    N = Nutation(tTT_century);    // epsi_rad_, depsilon_rad_, dpsi_rad_ are updated in this proccedure
...
Matrix<3, 3> CelestialRotation::Nutation(const double(&tTT_century)[4])
{
  ...
  dpsi_rad_ = c_dpsi_rad_[0] * sin(O_rad) + c_dpsi_rad_[1] * sin(2 * Ld_rad) + c_dpsi_rad_[2] * sin(2 * O_rad) + c_dpsi_rad_[3] * sin(2 * L_rad) + c_dpsi_rad_[4] * sin(ls_rad);           // [rad]
  dpsi_rad_ = dpsi_rad_ + c_dpsi_rad_[5] * sin(lm_rad) + c_dpsi_rad_[6] * sin(2 * Ld_rad + ls_rad) + c_dpsi_rad_[7] * sin(2 * L_rad + lm_rad) + c_dpsi_rad_[8] * sin(2 * Ld_rad - ls_rad); // [rad]
...
Matrix<3, 3> CelestialRotation::PolarMotion(const double Xp, const double Yp)
{
  ...
  W[0][0] = 1.0;  W[0][1] = 0.0; W[0][2] = -Xp;
  W[1][0] = 0.0;  W[1][1] = 1.0; W[1][2] = -Yp;
  W[2][0] = Xp;   W[2][1] = Yp;  W[2][2] = 1.0;
