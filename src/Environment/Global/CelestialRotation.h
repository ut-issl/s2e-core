...
// class for describing the rotational motion of center objects，instantiated as
// private members of CelesInfo（ひとまず地球自転のみに対応）
// アルゴリズムは暫定的に，福島,"天体の回転運動理論入門講義ノート",2007,
// 及び，長沢,"天体の位置計算(増補版)",2001に従う.(IERS Conventions 2003)
...
private:
  ...
  void Init_CelestialRotation_As_Earth(const RotationMode rotation_mode,
                                       const std::string center_obj);
  Matrix<3, 3>
  AxialRotation(const double GAST_rad); // movement of the coordinate axes due
                                        // to rotation around the rotation axis
