...
Vector<N> GnssSat_coordinate::TrigonometricInterpolation(
  ...
  double w =
      2.0 * M_PI / (24.0 * 60.0 * 60.0) * 1.03; // coefficient of a day long
  Vector<N> res(0.0);
...
template <size_t N>
Vector<N>
GnssSat_coordinate::LagrangeInterpolation(const vector<double> &time_vector,
                                          const vector<Vector<N>> &values,
                                          double time) const {
  int n = time_vector.size();
  Vector<N> res(0.0);
...
pair<double, double> GnssSat_position::Init(vector<vector<string>> &file,
  ...
  for (int page = 0; page < (int)file.size(); ++page) {
    ...
    int line = 3;
    while (file.at(page).at(line).front() != '*')
      ++line;
  ...
  if (file_extension == ".sp3") {
    ...
    for (int page = 0; page < (int)file.size(); ++page) {
      ...
      for (int line = 0; line < (int)file.at(page).size(); ++line) {
        if (file.at(page).at(line).substr(0, 3) != "AS ")
          continue;
        ...
        if (end_unix_time - unix_time < 1e-4)
          break;
        if (!unixtime_vector_.at(sat_id).empty() &&
...
GnssSatellites::GnssSatellites(bool is_calc_enabled)
    ...
    : ofs_true("true.csv"), ofs_esti("esti.csv"), ofs_sa("sa.csv")
#endif
{
...
libra::Vector<3>
GnssSatellites::GetSatellitePositionEcef(const int sat_id) const {
...
libra::Vector<3>
GnssSatellites::GetSatellitePositionEci(const int sat_id) const {
  // sat_id is wrong or not valid
  if (sat_id >= GetNumOfSatellites() || !GetWhetherValid(sat_id)) {
    libra::Vector<3> res(0);
    return res;
...
double GnssSatellites::AddIonosphericDelay(const int sat_id,
  ...
  double altitude = 0.0;
  for (int i = 0; i < 3; ++i)
    altitude += pow(rec_position[i], 2.0);
  altitude = sqrt(altitude);
  altitude = altitude / 1000.0 - Earth_hemisphere; //[m -> km]
  if (altitude >= 1000.0)
    return 0.0; // there is no Ionosphere above 1000km
