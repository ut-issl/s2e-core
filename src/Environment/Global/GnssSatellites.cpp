...
void GnssSat_position::SetUp(const double start_unix_time, const double step_sec)
{
  ...
  for(int sat_id = 0;sat_id < all_sat_num_;++sat_id){
    ...
    int index = lower_bound(unixtime_vector_.at(sat_id).begin(), unixtime_vector_.at(sat_id).end(), start_unix_time) - unixtime_vector_.at(sat_id).begin();
...
void GnssSat_clock::Init(vector<vector<string>>& file, string file_extension, int interpolation_number, UR_KINDS ur_flag, pair<double, double> unix_time_period)
{
  interpolation_number_ = interpolation_number;
  ...
  if(file_extension == ".sp3"){
    ...
    for(int page = 0;page < (int)file.size();++page){
      ...
      for(int line = 0;line < (int)file.at(page).size();++line){
        ...
        time_tm->tm_sec = (int)(stod(s.at(7)) + 1e-4);  // for the numerical error, plus 1e-4. tm_sec is tobe int
        double unix_time = (double)mktime(time_tm);
...
{
  auto unix_time_period =  position_.Init(position_file, position_interpolation_method,
                                          position_interpolation_number, position_ur_flag);
  clock_.Init(clock_file, clock_file_extension,
              clock_interpolation_number, clock_ur_flag, unix_time_period);
}
