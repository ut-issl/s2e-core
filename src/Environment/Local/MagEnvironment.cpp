...
MagEnvironment::MagEnvironment(string fname,
  double mag_rwdev,
  double mag_rwlimit,
  double mag_wnvar)
  :fname_(fname),
...
{
  //初期化コンストラクタ
  for (int i = 0; i < 3; ++i) { Mag_i_[i] = 0; }
  for (int i = 0; i < 3; ++i) { Mag_b_[i] = 0; }
  set_file_path(fname_.c_str());
}

void MagEnvironment::CalcMag(double decyear,
  double side,
  ...
  Quaternion q_i2b)
{
  ...
  for (int i = 0; i < 3; ++i) { Mag_i_[i] = mag_i_array[i]; }
...
Vector<3> MagEnvironment::GetMag_i() const
{
  return Mag_i_;
}

//地磁気ベクトル(body座標系)[nT]の取得用関数
Vector<3> MagEnvironment::GetMag_b() const
{
  return Mag_b_;
}
