...
SRPEnvironment::SRPEnvironment(LocalCelestialInformation* local_celes_info)
  ...
  sun_radius_m_ =
      local_celes_info_->GetGlobalInfo().GetMeanRadiusFromName("SUN");
}
...
void SRPEnvironment::UpdatePressure() {
  ...
  pressure_ =
      solar_constant_ / c_ / pow(distance_sat_to_sun / astronomical_unit_, 2.0);
}
...
void SRPEnvironment::CalcShadowCoefficient(string shadow_source_name) {
  ...
  const Vector<3> r_sc2sun_eci = local_celes_info_->GetPosFromSC_i("SUN");
  const Vector<3> r_sc2source_eci =
      local_celes_info_->GetPosFromSC_i(shadow_source_name.c_str());

  const double shadow_source_radius_m =
      local_celes_info_->GetGlobalInfo().GetMeanRadiusFromName(
          shadow_source_name.c_str());

  const double distance_sat_to_sun = norm(r_sc2sun_eci);
  const double sd_sun =
      asin(sun_radius_m_ / distance_sat_to_sun);  // Apparent radius of the sun
  const double sd_source =
      asin(shadow_source_radius_m /
           norm(r_sc2source_eci));  // Apparent radius of the shadow source

  const double delta = acos(
      inner_product(r_sc2source_eci, r_sc2sun_eci - r_sc2source_eci) /
      norm(r_sc2source_eci) /
      norm(r_sc2sun_eci - r_sc2source_eci));  // Angle of deviation from shadow
                                              // source center to sun center
  const double x =
      (delta * delta + sd_sun * sd_sun - sd_source * sd_source) /
      (2.0 *
       delta);  // The angle between the center of the sun and the common chord
  const double y =
      sqrt(max(sd_sun * sd_sun - x * x,
  ...
  if (c < fabs(a - b) &&
      a <= b)  // The occultation is total (spacecraft is in umbra)
  {
    shadow_coefficient_ = 0.0;
  } else if (c < fabs(a - b) &&
             a > b)  // The occultation is partial but maximum
  {
    shadow_coefficient_ = 1.0 - (b * b) / (a * a);
  } else if (fabs(a - b) <= c && c <= (a + b))  // spacecraft is in penunbra
  {
    double A =
        a * a * acos(x / a) + b * b * acos((c - x) / b) -
        c * y;  // The area of the occulted segment of the apparent solar disk
