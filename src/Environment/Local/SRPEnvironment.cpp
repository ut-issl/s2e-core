...
void SRPEnvironment::UpdateAllStates(Vector<3> &earth_position_b,
                                     Vector<3> &sun_position_b) {
  if (!IsCalcEnabled)
    return;
  ...
  double sd_sun =
      asin(r_sun_ / distance_sat_to_sun); // Apparent radius of the sun
  double sd_earth =
      asin(r_earth_ / norm(earth_position_b)); // Apparent radius of the earth
  double delta =
      acos(inner_product(earth_position_b, sun_position_b - earth_position_b) /
           norm(earth_position_b) /
           norm(sun_position_b -
                earth_position_b)); //地球中心から太陽中心のずれの角度
  double x =
      (delta * delta + sd_sun * sd_sun - sd_earth * sd_earth) /
      (2.0 *
       delta); // The angle between the center of the sun and the common chord
  double y = sqrt(max(sd_sun * sd_sun - x * x,
                      0.0)); // The length of the common chord of the apparent
                             // solar disk and apparent tellestial disk

  CalcShadowFunction(sd_sun, sd_earth, delta, x, y);

  for (int i = 0; i < 3; i++)
    d_sc2sun_b_[i] = sun_position_b[i] / distance_sat_to_sun;
}
...
double SRPEnvironment::GetShadowFunction() const { return shadow_function_; }
...
void SRPEnvironment::CalcShadowFunction(double a, double b, double c, double x,
                                        double y) {
  if (c < fabs(a - b) &&
