...
class LocalCelestialInformation : public ILoggable {
 public:
  ...
  void UpdateAllObjectsInfo(const Vector<3> sc_pos_from_center_i,
                            const Vector<3> sc_vel_from_center_i,
                            Quaternion q_i2b, const Vector<3> sc_body_rate);
  ...
  inline const CelestialInformation& GetGlobalInfo() const {
    return *glo_celes_info_;
  }
  // FOR LOG OUTPUT
...
void Convert_i2b_velocity(const double* r_i, const double* v_i, double* v_b,
                          const Quaternion q_i2b, const Vector<3> bodyrate_b);
