...
libra::Matrix<6, 6> CalcCarterStm(double orbit_radius_m, double gravity_constant_m3_s2, double f_ref_rad, double f_ref_0_rad, OrbitalElements* reference_oe){
  libra::Matrix<6, 6> stm;
  libra::Matrix<6, 6> stm_inv;
  // ここでstmを計算してください
  double n = sqrt(gravity_constant_m3_s2 / pow(orbit_radius_m, 3));
  double e = reference_oe->GetEccentricity();
  double E_rad = 2 * atan(sqrt((1 - e) / (1 + e)) * tan(f_ref_rad / 2));
  double k = 1 + e * cos(f_ref_rad);
  ...
  double phi1 = sin(f_ref_rad) * (e * cos(f_ref_rad) + 1);
  double sigma1 = atan(tan(f_ref_rad / 2) * sqrt(-(e - 1) / (e + 1)));
  double term1_phi2 = 2 * e * sin(f_ref_rad) * (e * cos(f_ref_rad) + 1);
  double term2_phi2 = (e * pow(sin(2 * sigma1), 3) / 3 + sin(4 * sigma1) / 4 - sigma1) / pow(1 - pow(e, 2), 5.0 / 2.0);
  double term3_phi2 = sin(f_ref_rad) / pow(e * cos(f_ref_rad) + 1, 3);
  double term4_phi2 = cos(f_ref_rad) / (e * cos(f_ref_rad) + 1);
  double phi2 = term1_phi2 * (term2_phi2 + term3_phi2) - term4_phi2;
  double sigma2 = e * cos(f_ref_rad) + 1;
  double term1_phi3 = pow(cos(f_ref_rad), 2);
  double term2_phi3 = 6 * e * sin(f_ref_rad) * sigma2 * (e * pow(sin(2 * sigma1), 3) / 3 + sin(4 * sigma1) / 4 - sigma1) / pow(1 - pow(e, 2), 5.0 / 2.0);
  double phi3 = -term1_phi3 - (term1_phi3 / sigma2) - (2 * pow(sin(f_ref_rad), 2) / pow(sigma2, 2)) - term2_phi3;
  double phi1_prime = cos(f_ref_rad) * (e * cos(f_ref_rad) + 1) - e * pow(sin(f_ref_rad), 2);
  double sigma4 = atan(tan(f_ref_rad / 2) * sqrt(-(e - 1) / (e + 1)));
  double term1_phi2_prime = sin(f_ref_rad) / sigma2;
  double term2_phi2_prime = 2 * e * sin(f_ref_rad) * sigma2 * (cos(f_ref_rad) / pow(sigma2, 3)) - (cos(f_ref_rad) / pow(1 - pow(e, 2), 5.0 / 2.0));
  double phi2_prime = term1_phi2_prime + term2_phi2_prime;
  double phi3_prime = sigma2 - 4 * e * pow(sin(f_ref_rad), 3) / pow(sigma1, 3) + 2 * cos(f_ref_rad) * sin(f_ref_rad) / pow(sigma1, 2) - 6 * e * cos(f_ref_rad) * sigma1 / pow(1 - pow(e, 2), 5.0 / 2.0);
