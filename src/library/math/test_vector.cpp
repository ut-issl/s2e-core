...
TEST(Vector, ConstructorWithNumber) {
  ...
  libra::Vector<N> v(initialize_value);
...
TEST(Vector, OperatorMinus) {
  ...
  libra::Vector<N> subtracted = v - subtracting;
...
TEST(Vector, OuterProductZero) {
  ...
  libra::Vector<3> result = OuterProduct(a, b);
...
TEST(Vector, GenerateOrthogonalUnitVector) {
  const size_t N = 3;
  libra::Vector<N> a(1.0);

  libra::Vector<N> b = GenerateOrthogonalUnitVector(a);
