...
MagDisturbance::MagDisturbance(const Vector<3> &rmm_const_b,
                               const double rmm_rwdev, const double rmm_rwlimit,
                               const double rmm_wnvar)
    : rmm_const_b_(rmm_const_b), rmm_rwdev_(rmm_rwdev),
      rmm_rwlimit_(rmm_rwlimit), rmm_wnvar_(rmm_wnvar) {
  for (int i = 0; i < 3; ++i) {
    torque_b_[i] = 0;
  }                   //残留磁気トルク値の初期化
...
void MagDisturbance::Update(const LocalEnvironment &local_env,
...
void MagDisturbance::PrintTorque() {
  cout << "MgDist_Torque_b =(" << torque_b_[0] << "," << torque_b_[1] << ","
       << torque_b_[2] << ") Nm";
  cout << endl;
}
