...
Disturbances::Disturbances(SimulationConfig* sim_config, const int sat_id,
                           Structure* structure) {
...
void Disturbances::Update(const LocalEnvironment& local_env,
                          const Dynamics& dynamics, const SimTime* sim_time) {
...
void Disturbances::InitializeInstances(SimulationConfig* sim_config,
  ...
  AirDrag* air_dist =
      new AirDrag(InitAirDrag(ini_fname_, structure->GetSurfaces(),
                              structure->GetKinematicsParams().GetCGb()));
  SolarRadiation* srp_dist =
      new SolarRadiation(InitSRDist(ini_fname_, structure->GetSurfaces(),
                                    structure->GetKinematicsParams().GetCGb()));
  MagDisturbance* mag_dist = new MagDisturbance(
      InitMagDisturbance(ini_fname_, structure->GetRMMParams()));
  ...
  ThirdBodyGravity* thirdbodygravity = new ThirdBodyGravity(
      InitThirdBodyGravity(ini_fname_, sim_config->ini_base_fname_));
