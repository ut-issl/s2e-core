...
GGDist::GGDist()
  : GGDist(3.986004418 * pow(10.0, 14.0))
{//デフォルトコンストラクタ
}

GGDist::GGDist(const double mu_e_input)
{//コンストラクタ
...
{//トルクを取得、引数は前から地球半径、地球中心方向単位ベクトル、慣性テンソル。引数の単位はそれぞれm,無次元,kg*m^2
  torque_b_ = 3.0 * mu_e_ / pow(R0, 3.0) * outer_product(normalize(u_b), I_b * normalize(u_b));
  return torque_b_;
}

Vector<3> GGDist::CalcTorque(Vector<3> r_b, Matrix<3, 3> I_b)
{//トルクを取得、引数は前から地球中心方向ベクトル(大きさ付き)、慣性テンソル。引数の単位はそれぞれm,kg*m^2
...
/*int main(){
    ...
    for(int i = 6400;i < 10000;i++){
        double R = double(i);
        Vector<3> r = R * u;
        log << gg.GetTorque(R,u,I)[0] << "," << gg.GetTorque(R,u,I)[1] << "," << gg.GetTorque(R,u,I)[2] << endl;
    }
}*/
