...
class AirDrag : public SurfaceForce {
 public:
   ...
   */
  AirDrag(const std::vector<Surface>& surfaces, const s2e::math::Vector<3>& center_of_gravity_b_m, const double wall_temperature_K,
 ...
 private:
   ...
   * @param [in] velocity_b_m_s: Spacecraft's velocity vector in the body frame [m/s]
   */
  void CalcCnCt(const s2e::math::Vector<3>& velocity_b_m_s);
  /**
   * @fn CalcFunctionPi
