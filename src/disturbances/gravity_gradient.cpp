...
void GravityGradient::Update(const LocalEnvironment& local_environment, const dynamics::Dynamics& dynamics) {
...
s2e::math::Vector<3> GravityGradient::CalcTorque_b_Nm(const s2e::math::Vector<3> earth_position_from_sc_b_m, const s2e::math::Matrix<3, 3> inertia_tensor_b_kgm2) {
  ...
  s2e::math::Vector<3> u_b = earth_position_from_sc_b_m;  // TODO: make undestructive normalize function for Vector
  u_b /= r_norm_m;
