...
void MagneticDisturbance::CalcRMM() {
  static libra::Vector<3> random_walk_std_dev(rmm_params_.GetRandomWalkStandardDeviation_Am2());
  static libra::Vector<3> random_walk_limit(rmm_params_.GetRandomWalkLimit_Am2());
  static RandomWalk<3> random_walk(0.1, random_walk_std_dev, random_walk_limit);  // [FIXME] step width is constant
  static libra::NormalRand normal_random(0.0, rmm_params_.GetRandomNoiseStandardDeviation_Am2(), global_randomization.MakeSeed());
