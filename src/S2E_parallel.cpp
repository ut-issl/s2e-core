...
//  concurrency::parallel_for(0, mc_sim->GetTotalNumOfExecutions(), //(auto i=0; i<mc_sim->GetTotalNumOfExecutions(); i++)
...
void mail_loop(int idx)
{
  ...
  SimulationCase& simcase = EquuleusCase(*mc_sim, ini_fname, cosmos_wrapper, hw_msg); // 最終的にはnullptrではなくCOSMOSWrapperインスタンスへのポインタを渡すべき。
  //mclog.AddLoggable(&simcase);
  //if (mc_sim->GetNumOfExecutionsDone() == 0)
  //{
  //  mclog.WriteHeaders();
