...
MagSensor::MagSensor(int prescaler, ClockGenerator* clock_gen, Sensor& sensor_base, const int sensor_id, const Quaternion& q_b2c,
                     const GeomagneticField* magnet)
    : Component(prescaler, clock_gen), Sensor(sensor_base), sensor_id_(sensor_id), q_b2c_(q_b2c), magnet_(magnet) {}
MagSensor::MagSensor(int prescaler, ClockGenerator* clock_gen, PowerPort* power_port, Sensor& sensor_base, const int sensor_id,
                     const Quaternion& q_b2c, const GeomagneticField* magnet)
    : Component(prescaler, clock_gen, power_port), Sensor(sensor_base), sensor_id_(sensor_id), q_b2c_(q_b2c), magnet_(magnet) {}
MagSensor::~MagSensor() {}
