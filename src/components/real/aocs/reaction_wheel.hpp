...
class ReactionWheel : public Component, public ILoggable {
 public:
  ...
  ReactionWheel(const int prescaler, ClockGenerator* clock_generator, const int component_id, const double step_width_s,
                const double rotor_inertia_kgm2, const double max_torque_Nm, const double max_velocity_rpm, const s2e::math::Quaternion quaternion_b2c,
                const s2e::math::Vector<3> position_b_m, const double dead_time_s, const double time_constant_s,
   ...
   */
  const s2e::math::Vector<3> GetOutputTorque_b_Nm() const;
  /**
   * @fn GetJitterForce_b_N
   * @brief Return output force by jitter in the body fixed frame [N]
   */
  inline const s2e::math::Vector<3> GetJitterForce_b_N() const;
  /**
   * @fn GetDriveFlag
 ...
 protected:
  ...
  const s2e::math::Quaternion quaternion_b2c_;  //!< Quaternion from body frame to component frame
  const s2e::math::Vector<3> position_b_m_;     //!< Position of RW in the body fixed frame [m]
  s2e::math::Vector<3> rotation_axis_c_;        //!< Wheel rotation axis on the component frame. Constant as (0 0 1). (Output torque is minus direction)
  s2e::math::Vector<3> rotation_axis_b_;        //!< Wheel rotation vector in the body fixed frame.
  ...
  s2e::math::Vector<3> output_torque_b_Nm_{0.0};      //!< Output torque in the body fixed frame [Nm]
  s2e::math::Vector<3> angular_momentum_b_Nms_{0.0};  //!< Angular momentum of RW [Nms]
