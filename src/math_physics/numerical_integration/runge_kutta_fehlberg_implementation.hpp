...
template <size_t N>
s2e::math::Vector<N> RungeKuttaFehlberg<N>::CalcInterpolationState(const double sigma) const {
  // Calc k7 (slope after state update)
  s2e::math::Vector<N> state_7 =
      this->previous_state_ + this->step_width_ * (1.0 / 6.0 * this->slope_[0] + 1.0 / 6.0 * this->slope_[4] + 2.0 / 3.0 * this->slope_[5]);
  s2e::math::Vector<N> k7 = this->ode_.DerivativeFunction(this->current_independent_variable_, state_7);
