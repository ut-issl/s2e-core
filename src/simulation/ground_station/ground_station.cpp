...
void GroundStation::Initialize(const SimulationConfiguration* configuration, const unsigned int ground_station_id) {
  ...
  geodetic_position_ = s2e::geodesy::GeodeticPosition(latitude_deg * s2e::math::deg_to_rad, longitude_deg * s2e::math::deg_to_rad, height_m);
...
void GroundStation::Update(const environment::EarthRotation& celestial_rotation, const Spacecraft& spacecraft) {
  s2e::math::Matrix<3, 3> dcm_ecef2eci = celestial_rotation.GetDcmJ2000ToEcef().Transpose();
...
bool GroundStation::CalcIsVisible(const s2e::math::Vector<3> spacecraft_position_ecef_m) {
  s2e::math::Quaternion q_ecef_to_ltc = geodetic_position_.GetQuaternionXcxfToLtc();

  s2e::math::Vector<3> sc_pos_ltc = q_ecef_to_ltc.FrameConversion(spacecraft_position_ecef_m - position_ecef_m_);  // Satellite position in LTC frame [m]
  sc_pos_ltc = sc_pos_ltc.CalcNormalizedVector();
  s2e::math::Vector<3> dir_gs_to_zenith = s2e::math::Vector<3>(0);
  dir_gs_to_zenith[2] = 1;

  // Judge the satellite position angle is over the minimum elevation

  if (dot(sc_pos_ltc, dir_gs_to_zenith) > sin(elevation_limit_angle_deg_ * s2e::math::deg_to_rad)) {
    return true;
  } else {
    return false;
  }
