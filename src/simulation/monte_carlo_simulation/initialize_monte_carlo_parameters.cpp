...
double InitParameter::Generate1dUniform(double lb, double ub) { return lb + (*InitParameter::uniform_distribution_)(InitParameter::mt_) * (ub - lb); }

double InitParameter::Generate1dNormal(double mean, double std) { return mean + (*InitParameter::normal_distribution_)(InitParameter::mt_) * (std); }

void InitParameter::GenerateNoRandomization() { randomized_value_.clear(); }

void InitParameter::GenerateCartesianUniform() {
  ...
  for (unsigned int i = 0; i < mean_or_min_.size(); i++) {
    randomized_value_.push_back(InitParameter::Generate1dUniform(mean_or_min_[i], sigma_or_max_[i]));
  }
}

void InitParameter::GenerateCartesianNormal() {
  // Random variables following a normal distribution in Cartesian frame
  randomized_value_.clear();
  for (unsigned int i = 0; i < mean_or_min_.size(); i++) {
    randomized_value_.push_back(InitParameter::Generate1dNormal(mean_or_min_[i], sigma_or_max_[i]));
  }
}
...
void InitParameter::GenerateCircularNormalUniform() {
...
void InitParameter::GenerateCircularNormalNormal() {
  const static int dim = 2;
  if (mean_or_min_.size() < dim || sigma_or_max_.size() < dim) {
    throw "Config parameters dimension unmatched.";
  }
...
void InitParameter::CalcSphericalNormalUniformUniform(libra::Vector<3>& destination, double r_mean, double r_sigma, double theta_min,
  ...
  double r = InitParameter::Generate1dNormal(r_mean, r_sigma);
  double theta = acos(cos(theta_min) - (cos(theta_min) - cos(theta_max)) * InitParameter::Generate1dUniform(0.0, 1.0));
  double phi = InitParameter::Generate1dUniform(phi_min, phi_max);
...
void InitParameter::CalcSphericalNormalNormal(libra::Vector<3>& destination, const libra::Vector<3>& mean_vec) {
  ...
  double rotation_angle_of_normal_unit_vec = InitParameter::Generate1dUniform(0.0, libra::tau);
  ...
  double rotation_angle_of_mean_vec = InitParameter::Generate1dNormal(0.0, sigma_or_max_[1]);
  ...
  ret_vec = InitParameter::Generate1dNormal(CalcNorm(mean_vec), sigma_or_max_[0]) * ret_vec;  // multiply norm
...
void InitParameter::GenerateSphericalNormalNormal() {
  const static int dim = 3;
...
void InitParameter::CalcQuaternionUniform(libra::Quaternion& destination) {
  ...
  double theta = acos(1 - (1 - (-1)) * InitParameter::Generate1dUniform(0.0, 1.0));
  double phi = InitParameter::Generate1dUniform(0, libra::tau);
  ...
  double rotation_angle = InitParameter::Generate1dUniform(0.0, libra::tau);
...
void InitParameter::GenerateQuaternionNormal() {
