...
Node::Node(const int node_id, const string node_label, const int heater_node_id,
    ...
    : node_id_(node_id), node_label_(node_label),
      heater_node_id_(heater_node_id), temperature_(temperature_ini),
      capacity_(capacity_ini), internal_heat_(internal_heat_ini), alpha_(alpha),
      area_(area), normal_v_b_(normal_v_b) {
  solar_radiation_ = 0;
}
