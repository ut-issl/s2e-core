...
Node::Node(const int node_id,
	...
	const double internal_heat_ini,
	const double alpha,
	const double area,
	Vector<3> normal_v_b
):
...
int Node::GetNodeId(void) const {
	return node_id_;
}

string Node::GetNodeLabel(void) const {
	return node_label_;
}

int Node::GetHeaterNodeId(void) const {
	return heater_node_id_;
}

double Node::GetTemperature_K(void) const
{
	return temperature_;
}
...
double Node::GetCapacity(void) const
{
	return capacity_;

}

double Node::GetInternalHeat(void) const
{
	return internal_heat_;
}

double Node::GetSolarRadiation(void) const {
	return solar_radiation_;
}

void Node::SetTemperature_K(double temp_K)
{
	temperature_ = temp_K;
}
...
{   
	...
	// calculate sun_power
	if(cos_theta > 0) solar_radiation_ = S * area_ * alpha_ * cos_theta;
	else solar_radiation_ = 0;
