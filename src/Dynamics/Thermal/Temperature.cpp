...
void Temperature::RungeOneStep(double t,double dt, Vector<3> sun_direction, int node_num)
{
	...
	for (int i = 0; i < node_num; i++) { x[i] = vnodes_[i].GetTemperature_K(); }
	...
	for (int i = 0; i < node_num; i++) { vnodes_[i].SetTemperature_K(next_x[i]); }
}
...
vector<Node> Temperature::GetVnodes() const {
	return vnodes_;
}
...
string Temperature::GetLogHeader() const
{
  ...
  /*
  下記のコードのようにiteratorでアクセスすると, インスタンス化されていないメンバ関数のアドレスを取っていることになり
  うまくいかない
  for (auto itr = vnodes_.cbegin() + 1; itr != vnodes_.cend(); ++itr) {
	  string str_node = "temp_" + to_string((*itr->GetNodeId)()) + " (" + to_string((*itr->GetNodeLabel)()) + ")";
	  str_tmp += WriteScalar(str_node, "[deg]");
  }*/
  return str_tmp;
}
