...
Dynamics::Dynamics(SimulationConfig *sim_config, const SimTime *sim_time,
...
void Dynamics::Initialize(SimulationConfig *sim_config, const SimTime *sim_time,
                          const LocalCelestialInformation *local_celes_info,
                          const int sat_id, Structure *structure,
                          RelativeInformation *rel_info) {
  ...
  // Initialize
  string center_body_name =
      local_celes_info->GetGlobalInfo().GetCenterBodyName();
  orbit_ = InitOrbit(&(local_celes_info->GetGlobalInfo()),
                     sim_config->sat_file_[sat_id],
                     sim_time->GetOrbitRKStepSec(), sim_time->GetCurrentJd(),
                     local_celes_info->GetGlobalInfo().GetGravityConstant(
                         center_body_name.c_str()),
                     "ORBIT", rel_info);
  attitude_ =
      InitAttitude(sim_config->sat_file_[sat_id], orbit_, local_celes_info,
                   sim_time->GetAttitudeRKStepSec(),
                   structure->GetKinematicsParams().GetInertiaTensor(), sat_id);
  temperature_ = InitTemperature(sim_config->sat_file_[sat_id],
                                 sim_time->GetThermalRKStepSec());

  // To get initial value
  orbit_->UpdateAtt(attitude_->GetQuaternion_i2b());
}

void Dynamics::Update(const SimTime *sim_time,
                      const LocalCelestialInformation *local_celes_info) {
  ...
  if (sim_time->GetThermalPropagateFlag()) {
    ...
    temperature_->Propagate(local_celes_info->GetPosFromSC_b(c_sun),
                            sim_time->GetElapsedSec());
    delete[] c_sun;
  }
}
