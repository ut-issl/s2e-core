...
ordered_force_b_N = read_3d_vector_from_csv(read_file_name, 'ideal_torque_generator_ordered_torque_b', 'Nm')
generated_force_b_N = read_3d_vector_from_csv(read_file_name, 'ideal_torque_generator_generated_torque_b', 'Nm')
...
fig.suptitle("Torque Generator Output @ Body frame")
fig.supylabel("Torque" + unit)
fig.supxlabel("Time [s]")

# Data save
if args.no_gui:
  plt.savefig(read_file_tag + "_torque_generator.png") # save last figure only
else:
  plt.show()
