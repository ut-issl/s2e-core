...
measured_magnetic_field_c_nT = read_3d_vector_from_csv(read_file_name, 'magnetometer1_measured_magnetic_field_c', 'nT')
true_magnetic_field_b_nT = read_3d_vector_from_csv(read_file_name, 'geomagnetic_field_at_spacecraft_position_b', 'nT')

# Statistics
# We assume that the component frame and the body frame is same
error_rad_s = measured_magnetic_field_c_nT - true_magnetic_field_b_nT
average = [0.0, 0.0, 0.0]
...
# Data save
if args.no_gui:
  plt.savefig(read_file_tag + "_magnetometer.png") # save last figure only
else:
  plt.show()
